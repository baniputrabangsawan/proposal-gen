...
var revalidateLogic = ({ mode = "submit", modeAfterSubmission = "change" } = {}) => (props) => {
	...
	if ([(props.group ? props.group.state.meta.submissionAttempts : props.form.state.submissionAttempts) === 0 ? mode : modeAfterSubmission, "submit"].includes(props.event.type)) validatorsToAdd.push(dynamicValidator);
		...
		this.validateAsync = async (cause, validateOpts) => {
			...
			for (const validateObj of validates) {
				...
				promises.push(new Promise(async (resolve) => {
					...
					const errorMapKey = getErrorMapKey$2(validateObj.cause);
					const allFieldsToProcess = /* @__PURE__ */ new Set([...Object.keys(this.state.fieldMeta), ...Object.keys(fieldErrorsFromFormValidators || {})]);
					...
					for (const field of fields) {
						if (this.baseStore.state.fieldMetaBase[field] === void 0 && !fieldErrorsFromFormValidators?.[field]) continue;
						const { errorMap: currentErrorMap, errorSourceMap: currentErrorMapSource } = this.getFieldMeta(field) ?? defaultFieldMeta;
	...
	*/
	constructor(opts) {
		...
		this.validateAsync = async (cause, formValidationResultPromise) => {
			...
			const linkedFieldsWithAsyncValidators = Array.from(new Set(linkedFieldValidates.filter((v) => v.validate).map((v) => v.field)));
			batch(() => {
...
//#endregion
export { Field, FieldApi, FieldGroupApi, FormApi, FormGroup, FormGroupApi, concatenatePaths, createFieldMap, createFormHook, createFormHookContexts, deepCopy, defaultValidationLogic, deleteBy, determineFieldLevelErrorSourceAndValue, determineFormLevelErrorSourceAndValue, evaluate, formEventClient, formOptions, functionalUpdate, getAsyncValidatorArray, getBy, getDefaultFormGroupMeta, getSyncValidatorArray, isFieldInGroup, isGlobalFormValidationError, isNonEmptyArray, isStandardSchemaValidator, makePathArray, mergeAndUpdate, mergeForm, mergeOpts, mutateMergeDeep, revalidateLogic, setBy, standardSchemaValidators, throttleFormState, useField, useFieldGroup, useForm, useFormGroup, useIsomorphicLayoutEffect, useSelector, useStore, uuid };

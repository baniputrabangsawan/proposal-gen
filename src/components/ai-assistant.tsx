...
export function AiAssistant({ proposal, onChange }: { proposal: Proposal; onChange: (proposal: Proposal) => void }) {
  ...
  return <Card title="AI Proposal Assistant"><div className="grid gap-3"><TextArea label="Catatan kasar" placeholder="Buat website company profile... harga Rp15 juta" value={notes} onChange={(event) => setNotes(event.target.value)} /><div className="flex flex-wrap gap-2">{actions.map((action) => <Button key={action} variant={action === "Generate Proposal" ? "primary" : "secondary"} onClick={() => run(action)}><Sparkles size={14} />{action}</Button>)}</div>{status && <p className="rounded-lg bg-slate-100 px-3 py-2 text-sm font-medium text-slate-900 dark:bg-neutral-900 dark:text-neutral-100">{status}</p>}<p className="text-xs text-slate-500">Provider-agnostic stub: siap diganti API OpenAI, Anthropic, Gemini, atau provider lain via backend/proxy aman.</p></div></Card>;
}

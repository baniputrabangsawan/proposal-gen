...
export default function App() {
  ...
  return <div className="min-h-screen bg-slate-100 text-slate-900">
    <ProposalHeader lastSaved={lastSaved} onNew={newProposal} onSave={saveNow} onPreview={showPreview} onExport={exportPdf} />
    {notice && <div className="mx-auto mt-3 max-w-[1600px] px-4"><div className="flex items-center justify-between gap-3 rounded-lg border border-indigo-200 bg-indigo-50 px-4 py-2 text-sm font-medium text-indigo-900"><span>{notice}</span>{previewOnly && <button type="button" className="font-bold underline" onClick={() => setPreviewOnly(false)}>Kembali ke editor</button>}</div></div>}
    <main className={previewOnly ? "mx-auto grid max-w-[1000px] gap-4 px-4 py-4" : "mx-auto grid max-w-full gap-4 px-4 py-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] xl:grid-cols-[minmax(0,640px)_minmax(0,1fr)]"}>
      ...
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className={mobileTab === "editor" && !previewOnly ? "hidden min-w-0 lg:block" : "block min-w-0"}><div className="sticky top-20 max-h-[calc(100vh-6rem)] overflow-auto rounded-xl border border-slate-200 bg-slate-50 p-3"><ProposalPreview proposal={proposal} /></div></motion.div>
    </main>
  </div>;
}
